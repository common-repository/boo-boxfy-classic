		...
		<form method="post" id="boo_mail_options">
			<table class="form-table">
				<tbody>
					<tr valign="top">
						<th scope="row">
							<label for="boo_boomail"><?php echo __('e-mail', 'booboxfy'); ?>:</label>
						</th>
						<td>
...
if (!get_option('boo_booid') && get_option('admin_email')) {
	function booboxfy_config_automatic() {
		if ($_GET['page'] != 'boobox-config') {
			echo "<div id='boo-warning' class='updated fade'><p><strong>".__('boo-box is almost ready', 'booboxfy').".</strong> <a href='options-general.php?page=boobox-config'>Your boo-boxfy has been automatic configured. Please check your email.</a></p></div>";
		}
